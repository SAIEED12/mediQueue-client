...
const ExtraSection = () => {
    ...
    const stats = [
        { icon: <Users className="w-5 h-5 text-[#0070c9]" />, value: "5,000+", label: "Active Students" },
        { icon: <ShieldCheck className="w-5 h-5 text-[#00b4d8]" />, value: "1,200+", label: "Verified Tutors" },
        { icon: <CheckCircle className="w-5 h-5 text-[#0070c9]" />, value: "15,000+", label: "Sessions Completed" },
        { icon: <Star className="w-5 h-5 text-amber-500 fill-current" />, value: "4.9/5", label: "Average Rating" }
    ];

    return (
        <div className="space-y-24 py-16 bg-slate-50/50">
            ...
            <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-16 relative">
                    {steps.map((step, idx) => (
                        <div 
                            key={idx} 
                            className="relative bg-white border border-slate-100 p-8 rounded-3xl transition-all duration-300 hover:shadow-xl flex flex-col items-center text-center group"
                        >
            ...
            <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="bg-white border border-slate-200 rounded-4xl p-8 md:p-12 shadow-sm relative overflow-hidden">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center relative z-10">
                        ...
                        <div className="lg:col-span-7 grid grid-cols-2 gap-4 md:gap-6">
                            {stats.map((stat, idx) => (
                                <div 
                                    key={idx} 
                                    className="bg-slate-50 border border-slate-100 p-6 rounded-2xl flex flex-col items-center justify-center text-center transition-colors hover:bg-slate-100/50"
                                >
                                    <div className="p-2 bg-white rounded-xl border border-slate-100 shadow-sm mb-3">
                                        {stat.icon}
                                    </div>
                                    <span className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight">
                                        {stat.value}
                                    </span>
                                    <span className="text-xs font-bold text-slate-400 uppercase tracking-wide mt-0.5">
                                        {stat.label}
                                    </span>
                                </div>
                            ))}
