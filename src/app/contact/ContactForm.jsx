            ...
            <div className="max-w-6xl mx-auto w-full space-y-10">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
                    ...
                    <div className="lg:col-span-5 flex flex-col justify-between gap-4">
                        <div className="flex items-start gap-4 p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-xs">
                        ...
                        <div className="flex items-start gap-4 p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-xs">
                            ...
                            <div className="space-y-1">
                                ...
                                <p className="text-sm font-black text-slate-700 dark:text-slate-300 pt-1">Dhaka, Bangladesh</p>
                            </div>
                        </div>

                        <div className="flex items-center gap-3 p-5 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-900/40 rounded-3xl">
                        ...
                        <div className="bg-white dark:bg-slate-900 p-8 sm:p-10 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-2xl relative overflow-hidden">
                            ...
                            <form className="space-y-6 relative" onSubmit={handleSubmit}>
                                <div className="space-y-2 flex flex-col">
                                    ...
                                    <input id="name" required placeholder="John Doe" type="text" name="name" className={`${inputStyleClasses} h-14 px-5`} />
                                </div>

                                <div className="space-y-2 flex flex-col">
                                    <label htmlFor="email" className="text-sm font-bold text-slate-700 dark:text-slate-300 ml-1">Email Address</label>
                                    <input id="email" required placeholder="john@example.com" type="email" name="email" className={`${inputStyleClasses} h-14 px-5`} />
                                </div>
                                ...
                                <div className="space-y-2 flex flex-col">
                                    ...
                                    <div className="relative w-full">
                                        ...
                                        <input id="subject" required placeholder="How can we help you?" type="text" name="subject" className={`${inputStyleClasses} h-14 pl-12 pr-5`} />
                                    </div>
                                </div>

                                <div className="space-y-2 flex flex-col">
                                    <label htmlFor="message" className="text-sm font-bold text-slate-700 dark:text-slate-300 ml-1">Your Message</label>
                                    <textarea id="message" required placeholder="Type your message here in detail..." name="message" className={`${inputStyleClasses} min-h-[140px] p-4 resize-none`} />
                                </div>

                                <button type="submit" disabled={isLoading} className="w-full h-14 text-lg font-black rounded-2xl shadow-xl bg-gradient-to-r from-[#0070c9] to-[#00b4d8] hover:opacity-95 text-white shadow-blue-500/10 transition-transform duration-200 hover:scale-[1.01] active:scale-[0.99] group mt-2 flex items-center justify-center gap-2 disabled:opacity-70 cursor-pointer">
                                    {isLoading ? (
                                        <span className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                                    ) : (
                                        <>Send Message <Send className="w-5 h-5 ml-1 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" /></>
                                    )}
                                </button>
                            </form>
                        </div>
